...
with open("model_features.json") as file:
    ...
    for route in data.keys():
        ...
        db_entry = {"ID": route, "features": json.dumps(data[route])}

        # build an sql query to insert json data to db
        sql = db.construct_sql(table_name="model_features", query_type="insert", data=db_entry)
